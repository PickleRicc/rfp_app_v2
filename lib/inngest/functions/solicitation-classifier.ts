...
1. base_rfp — The master solicitation document that contains submission instructions, evaluation criteria, and/or proposal structure. This includes ANY primary solicitation vehicle regardless of what it is called: RFP (Request for Proposal), RFQ (Request for Quotation), TOR (Task Order Request), TORP (Task Order Request Package), fair opportunity notice, or any other document that tells offerors what to submit and how proposals will be evaluated. Key signals: contains Section L/M, Instructions to Offerors, evaluation factors, proposal requirements, or submission deadlines. If a document functions as the "master solicitation" — even if it also contains a SOW/PWS section embedded within it — classify it as base_rfp, NOT soo_sow_pws. IMPORTANT: A full-length TOR or RFP document is STILL base_rfp even if its title or header says "Amendment 0001" — what matters is that the document contains the COMPLETE solicitation content (instructions, evaluation factors, CLINs, etc.), not just a list of changes. Agencies often reissue the entire solicitation document with amendments incorporated.
2. soo_sow_pws — A STANDALONE Statement of Objectives (SOO), Statement of Work (SOW), or Performance Work Statement (PWS) that is a separate attachment from the base solicitation. Key signals: describes the work to be performed, task areas, deliverables, objectives, performance requirements, or technical scope. Do NOT use this type if the SOW/PWS is embedded inside the main solicitation document — use base_rfp instead. IMPORTANT: A full-length SOO/SOW/PWS document is STILL soo_sow_pws even if its title says "Amendment 0001" or "Revised" — what matters is that the document contains the COMPLETE scope of work (all task areas, all requirements, full performance details), not just a list of changes. Agencies commonly reissue the entire SOO/SOW/PWS with amendments incorporated rather than issuing a separate change document.
3. amendment — A SHORT formal amendment or modification notice whose PRIMARY PURPOSE is to LIST CHANGES to previously issued solicitation terms. Key signals: Standard Form 30 (SF30), a brief cover page describing what sections changed, or a short document that says "the following changes are hereby made." CRITICAL DISTINCTION: If a document is a FULL-LENGTH reissued solicitation, SOO, SOW, or PWS (typically 10+ pages of substantive content) that happens to say "Amendment" in the title, it is NOT an amendment — classify it by its actual content type (base_rfp or soo_sow_pws). The amendment type is ONLY for short modification notices (typically 1-5 pages) that describe WHAT CHANGED, not for complete replacement documents with amendments incorporated. When in doubt: if the document reads like a complete SOO/SOW/PWS with task areas and requirements, it is soo_sow_pws. If it reads like a complete solicitation with evaluation criteria and instructions, it is base_rfp.
