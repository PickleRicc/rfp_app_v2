...
/**
 * POST /api/intake/[token]/past-performance/upload
 * Public. Upload a past performance file and AI-extract records via intake token.
 */
export async function POST(
  ...
  try {
    ...
    try {
      ...
      if (insertError) {
        console.error(
          "Error inserting extracted past performance:",
          insertError
        );
        await supabase
          .from("past_performance_files")
    ...
    console.error("Past performance upload via token error:", error);
    return NextResponse.json(
